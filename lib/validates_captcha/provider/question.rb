    ...
    class Question
      ...
      def render_regenerate_challenge_link(sanitized_object_name, object, options = {}, html_options = {})
        ...
        if options.delete(:jquery)
          onclick = "var timestamp = new Date().getTime(); jQuery.getJSON('#{regenerate_path}?' + timestamp, function(result){jQuery('##{sanitized_object_name}_captcha_question').text(result.captcha_challenge); jQuery('##{sanitized_object_name}_captcha_challenge').val(result.captcha_challenge); jQuery('##{sanitized_object_name}_captcha_solution').val('');});return false;"
